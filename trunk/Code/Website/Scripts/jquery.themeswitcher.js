...
$.fn.themeswitcher = function(settings) {
    ...
    var switcherpane = setupSwitcher();
    ...
    /* Theme Loading
    ---------------------------------------------------------------------*/
    switcherpane.find('li').click(function() {
        updateCSS($(this).text());
    ...
    function setupSwitcher() {
        var _select = '<div class="jquery-ui-themeswitcher"><div id="themeGallery">	<ul>';
        for (var i = 0; i < options.themes.length; i++) {
            _select += "<li>" + options.themes[i] + "</li>";
        }
        _select += '</ul></div></div>';
        return $(_select).find('div').removeAttr('id');
    }

    //function to apply a new theme stylesheet
    function updateCSS(locStr) {
        console.log(locStr);
        $(options.cssId).attr('href',
                            'Content/themes/' +
                            locStr + '/jquery.ui.all.css');
    }
    ...
    if ($.cookie(options.cookieName) || options.loadTheme) {
        ...
        switcherpane.find('li:contains(' + themeName + ')').trigger('click');
    }
