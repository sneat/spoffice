...
function Themes() {
	...
	this._settings = {
		...
		useAmalgamationIcons: true, // True if we should use the amalgamated theme icons, false if we should use individual icons located in img/<theme-folder-name>.thumb.gif and img/<theme-folder-name>.thumb_small.gif
	...
	_updateTheme: function(target, settings) {
		...
		$.each(themes, function(index, id) {
			...
			html += '<li class="themes__' + id + ($.themes.currentTheme == id ? ' themes_current' : '') +
				...
				escape(theme.display) + '\',\'' + escape(settings.themeBase + theme.url + '/jquery.ui.all.css') + '\')"' +
			...
			if (theme.icon != null) {
				...
				if (typeof theme.icon == 'number') {
				    html += ' style="background: transparent url(' + (settings.useAmalgamationIcons ? settings.icons : settings.themeBase + 'img/' + theme.url + '.thumb_small.gif') +
						') no-repeat ' + (settings.useAmalgamationIcons ? '-' + (theme.icon * settings.iconSize[0]) : '0') + 'px 0px;' +
